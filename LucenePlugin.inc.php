...
class LucenePlugin extends GenericPlugin {
	...
	function register($category, $path) {
		...
		if (parent::register($category, $path)) {
			...
			// TODO: debug context id in getEnabled - script and website read different context id. should be sidewide
			if ($this->getEnabled()) {
	...
	function callbackRetrieveResults($hookName, $params) {
		...
		$results =& $params[11]; // need to use reference
	...
	*/
		...
		if (is_null($result)) {
			...
			if (isset($result['scoredResults']) && !empty($result['scoredResults'])) {
				...
				fwrite($debugFile, " LucenePlugin:scoredResults: ".print_r($result['scoredResults'], TRUE));
